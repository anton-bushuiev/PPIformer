...
class EGNN(nn.Module):
    def __init__(
            self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf=0, device=None,
            act_fn=nn.SiLU, n_layers=4, residual=True, attention=False,
            normalize=False, tanh=False, update_coords=True):
        '''
        ...
        gcl_params = dict(
            input_nf=self.hidden_nf, output_nf=self.hidden_nf, hidden_nf=self.hidden_nf,
            edges_in_d=in_edge_nf, act_fn=act_fn, residual=residual, attention=attention,
            normalize=normalize, tanh=tanh, update_coords=update_coords
        )
        for i in range(0, n_layers):
            if i == n_layers - 1 or not update_coords:
                gcl_params['update_coords'] = False
            self.add_module("gcl_%d" % i, E_GCL(**gcl_params))
